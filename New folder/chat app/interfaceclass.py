...
"""
    ...
    def classify_dark_pattern(self, defects, website):
        answer = self.chain.invoke(
            {"content": defects, "website": website, "dark_pattern": self.dark_patterns})
        return answer
...
def extract_arrays_from_input_string(input_string):
    ...
    content_match = re.search(
        r"<content>(.*?)</content>", input_string, re.DOTALL)
    dark_pattern_match = re.search(
        r"<dark_pattern>(.*?)</dark_pattern>", input_string, re.DOTALL)

    if content_match and dark_pattern_match:
        content_str = content_match.group(1).replace('\n', '').strip()
        dark_pattern_str = dark_pattern_match.group(
            1).replace('\n', '').strip()
